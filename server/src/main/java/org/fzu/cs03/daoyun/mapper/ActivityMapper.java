...
public interface ActivityMapper extends BaseMapper<AttendActivity> {
    ...
    @Select("select pa2.id activityId, pa2.activity_type_id activityTypeId, pia.user_id userId, pia.creation_date creationDate\n" +
            "from (SELECT pa.id , pa.activity_type_id from published_activity as pa  where pa.org_id = #{orgId} ) as pa2 LEFT JOIN \n" +
